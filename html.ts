...
function applyInlineCSS(document: Document) {
    ...
    const inline = {
        base: takeInlineCSS(document.body),
        rules: new Map(["narrow", "medium", "wide", "widest"].map(rule => [rule, takeInlineCSS(document.body, rule + "-")]))
    }

    const styles = { base: "", rules: {} as any }
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
    const classNames: string[] = []
    for (const [el, properties] of inline.base) {

        let className: string
        do {

            className = (Array.from({length:24}) as number[])
                .reduce((name) => name + chars.charAt(Math.floor(Math.random() * 24)), "")

        } while (!className || classNames.includes(className))

        classNames.push(className), el.classList.add(className)

        if (className == "fail")
            console.log(className), console.log(classNames);

        let css = ""
        for (const [name, value] of properties)
            css += name + ':' + value + ";"

        styles.base += `.${className}{${css}}\n`

        for (const name of inline.rules.keys()) {

            const properties = inline.rules.get(name)?.get(el)
            if (properties) {
                ...
                for (const [name, value] of properties)
                    ...
                    styles.rules[name] = (styles.rules[name] || "") + `.${className} {${css}}\n`
            }
        }
    }
